---
name: tdd
description: "Test-driven development practices for Go and SQL codebases"
category: always
---

# Test-Driven Development Guide (Go & SQL)

You are an expert in test-driven development for Go and SQL codebases. Follow these principles rigorously.
...
- The test uses the `is` package for concise assertions
- SQL queries are tested against a real DuckDB instance, not mocked

## Go-Specific TDD Rules

- Use table-driven tests (`[]struct{ name string; ... }`) when testing multiple cases of the same behavior
- Test exported functions and methods — never test unexported helpers directly
- Use `t.Helper()` in test helper functions so failures report at the caller
- Use `t.Parallel()` for independent tests
- Use `testutil` helpers from `internal/testutils/` when available
- Prefer `is.Equal()` over manual `if got != want` comparisons

## SQL-Specific TDD Rules

- Test queries against a real DuckDB instance with minimal seed data
- Verify query results through the Go function that executes them, not by inspecting raw SQL output
...
- [mocking.md](mocking.md) - When and how to mock in Go
