// Package embeddedpg runs a throwaway Postgres instance for tests, so
// `go test ./...` needs no locally-installed or running Postgres.
package embeddedpg
...
func portConflict(port uint32, dataDir string) error {
	...
	return fmt.Errorf(
		"embeddedpg: %w: %d is held by a postmaster that is not the one for %s "+
			"(another checkout, or a stale instance); stop it or configure a different port",
